...
Status do_encoding(EncodeInfo *encInfo)
{
	...
	{
		printf("Files opening successful completed\n");
		if (check_capacity(encInfo) == e_success)
		{
			printf("Check capacity is done completed\n");
			if (copy_bmp_header(encInfo->fptr_src_image, encInfo->fptr_stego_image) == e_success)
			{
				printf("Copy bmp header is done completed\n");
				if (encode_magic_string(MAGIC_STRING, encInfo) == e_success)
				{
					printf("Magic string encoded successfully completed\n");
					if (encode_secret_file_extn_size(strlen(encInfo->extn_secret_file), encInfo) == e_success)
					{
						printf("Secret file extn size encoded successfully completed\n");
						if (encode_secret_file_extn(encInfo->extn_secret_file, encInfo) == e_success)
						{
							printf("secret file extn encoded successfully completed\n");
							if (encode_secret_file_size(encInfo->size_secret_file, encInfo) == e_success)
							{
								printf("secret file size encoded successfully completed\n");
								if (encode_secret_file_data(encInfo) == e_success)
								{
									printf("secret file data encoded successfully completed\n");
									if (copy_remaining_img_data(encInfo->fptr_src_image, encInfo->fptr_stego_image) == e_success)
									{
										printf("Remaining data copied successfully completed\n");
										return e_success;
									}
									else
									{
...
Status open_files(EncodeInfo *encInfo)
{
	...
	// Do Error handling for Secret file
	...
	// Do Error handling for Stego Image file
...
Status check_capacity(EncodeInfo *encInfo)
{
	...
	printf("size of bmp file -> %d\n", encInfo->image_capacity);	  // printing size of BMP
	...
	if (((strlen(MAGIC_STRING) + 4 + 4 + len_of_extn + encInfo->size_secret_file) * 8) < get_image_size_for_bmp(encInfo->fptr_src_image)) // if the given size is less than input file size then we can do encoding
	{
		return e_success;
	}
	else
...
/*-------------------------------------------------------------------------------------------------------*/
// Function Defination Copy BMP Header
...
/*-------------------------------------------------------------------------------------------------------*/
// Function  defination Get File Size
...
/*-------------------------------------------------------------------------------------------------------*/
// FUnction Defination  Get Image Size for BMP
...
/*-------------------------------------------------------------------------------------------------------*/
// Function Defination Encode Magic String
...
/*-------------------------------------------------------------------------------------------------------*/
// Function Defination Encode Data to Image
Status encode_data_to_image(char *data, int size, FILE *fptr_src_image, FILE *fptr_stego_image) // data will be full string
{
	...
	{
		/*magic string contains characters so each time it will read 8 bytes of data from input image to encode 8 bits of data */
...
/*-------------------------------------------------------------------------------------------------------*/
// Function Defination encode byte to LSB
Status encode_byte_to_lsb(char data, char *image_buffer)
{
	unsigned int s = data;
	for (int i = 0; i < 8; i++)
	{
		// replacing the lsb bit each time based on decimal values of the characters
		image_buffer[i] = (image_buffer[i] & 0xFE) | (unsigned)((s & (1 << (7 - i))) >> (7 - i));
	}
	return e_success;
}
/*--------------------------------------------+-----------------------------------------------------------*/
...
/*-------------------------------------------------------------------------------------------------------*/
// Function Defination Encode SIze to LSB
Status encode_size_to_lsb(long extn_size, char *image_buffer)
{
	for (int i = 0; i < 32; i++)
	{
		/*to replacee the lsb bit for the size of the files we need 32 bytes of data because,
		 size is int for int there are 32 bits to encode each 32 bits of data we need 32 bytes */
		image_buffer[i] = (image_buffer[i] & 0xFE) | (unsigned)((extn_size & (1 << (31 - i))) >> (31 - i));
	}
	return e_success;
} /*-------------------------------------------------------------------------------------------------------*/

/*-------------------------------------------------------------------------------------------------------*/
// Function Defination Encode Secret File Extension
...
/*-------------------------------------------------------------------------------------------------------*/
// Function Defination Encode Secret File data
...
/*-------------------------------------------------------------------------------------------------------*/
// Function Defination Copy remaining Img Data
Status copy_remaining_img_data(FILE *fptr_src, FILE *fptr_dest)
{
	char ch;
	// copy remaining data after encoding the bits
